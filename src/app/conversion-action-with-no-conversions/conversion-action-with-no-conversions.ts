...
@Component({
  selector: 'app-conversion-action-with-no-conversions',
  standalone: true,
  imports: [CommonModule, FormsModule, HttpClientModule],
  templateUrl: './conversion-action-with-no-conversions.html',
  styleUrls: ['./conversion-action-with-no-conversions.css']
})
export class ConversionActionWithNoConversions implements OnInit {
  ...
  loadFileTypes(): void {
    // Load file types specific to the 'conversions' check
    const checkKey = 'conversions';
    ...
    this.http.get<any>(apiUrl, { withCredentials: true }).subscribe({
      next: (response) => {
        ...
        console.log('File types loaded for conversions check:', this.fileTypes);
      },
      error: (error) => {
