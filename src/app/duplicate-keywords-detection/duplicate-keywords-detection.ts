...
@Component({
  selector: 'app-duplicate-keywords-detection',
  standalone: true,
  imports: [CommonModule, FormsModule, HttpClientModule],
  templateUrl: './duplicate-keywords-detection.html',
  styleUrls: ['./duplicate-keywords-detection.css']
})
export class DuplicateKeywordsDetection implements OnInit {
  ...
  loadFileTypes(): void {
    // Load file types specific to the 'duplicate_keywords' check
    const checkKey = 'duplicate_keywords';
    ...
    this.http.get<any>(apiUrl, { withCredentials: true }).subscribe({
      next: (response) => {
        ...
        console.log('File types loaded for duplicate_keywords check:', this.fileTypes);
      },
      error: (error) => {
  ...
  runAudit() {
    ...
    if (this.dataSource === 'upload') {
      ...
      const check = 'duplicate_keywords';
      const requestBody = {
