...
@Component({
  selector: 'app-active-ad-groups-with-no-keywords',
  standalone: true,
  imports: [CommonModule, FormsModule, HttpClientModule],
  templateUrl: './active-ad-groups-with-no-keywords.html',
  styleUrls: ['./active-ad-groups-with-no-keywords.css']
})
export class ActiveAdGroupsWithNoKeywords implements OnInit {
  ...
  loadFileTypes(): void {
    // Load file types specific to the 'no_keywords' check
    const checkKey = 'no_keywords';
    ...
    this.http.get<any>(apiUrl, { withCredentials: true }).subscribe({
      next: (response) => {
        ...
        console.log('File types loaded for no_keywords check:', this.fileTypes);
      },
      error: (error) => {
