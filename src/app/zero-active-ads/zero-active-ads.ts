...
@Component({
  selector: 'app-zero-active-ads',
  templateUrl: './zero-active-ads.html',
  styleUrls: ['./zero-active-ads.css'],
  standalone: true,
  imports: [CommonModule, TitleCasePipe]
})
export class ZeroActiveAds implements OnInit {
  ...
  loadAdsData(): void {
    ...
    this.api.getScriptData(fileName, sheetName).subscribe({
      next: (response) => {
        ...
        this.adsData = response.Zeroactiveads || [];
        this.extractColumns();
        this.loading = false;
  ...
  exportToExcel(): void {
    ...
    try {
      ...
      const fileName = `zero-active-ad-${this.getCurrentDate()}.xlsx`;
