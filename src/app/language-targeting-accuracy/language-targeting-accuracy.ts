...
@Component({
  selector: 'app-language-targeting-accuracy',
  standalone: true,
  imports: [CommonModule, FormsModule, HttpClientModule],
  templateUrl: './language-targeting-accuracy.html',
  styleUrls: ['./language-targeting-accuracy.css']
})
export class LanguageTargetingAccuracy implements OnInit {
  ...
  loadFileTypes(): void {
    // Load file types specific to the 'lang' check
    const checkKey = 'lang';
    ...
    this.http.get<any>(apiUrl, { withCredentials: true }).subscribe({
      next: (response) => {
        ...
        console.log('File types loaded for lang check:', this.fileTypes);
      },
      error: (error) => {
