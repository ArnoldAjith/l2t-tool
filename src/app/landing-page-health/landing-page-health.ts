...
@Component({
  selector: 'app-landing-page-health',
  standalone: true,
  imports: [CommonModule, FormsModule, HttpClientModule],
  templateUrl: './landing-page-health.html',
  styleUrls: ['./landing-page-health.css']
})
export class LandingPageHealth implements OnInit {
  ...
  loadFileTypes(): void {
    // Load file types specific to the 'LandingPageHealth' check
    const checkKey = 'LandingPageHealth';
    ...
    this.http.get<any>(apiUrl, { withCredentials: true }).subscribe({
      next: (response) => {
        ...
        console.log('File types loaded for LandingPageHealth check:', this.fileTypes);
      },
      error: (error) => {
  ...
  /**
   * Fetch the detailed check data from the audit
   */
