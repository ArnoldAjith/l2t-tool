...
@Component({
  selector: 'app-active-ad-groups-with-no-ad-copy',
  standalone: true,
  imports: [CommonModule, FormsModule, HttpClientModule],
  templateUrl: './active-ad-groups-with-no-ad-copy.html',
  styleUrls: ['./active-ad-groups-with-no-ad-copy.css']
})
export class ActiveAdGroupsWithNoAdCopy implements OnInit {
  ...
  loadFileTypes(): void {
    // Load file types specific to the 'no_ad_copy' check
    const checkKey = 'no_ad_copy';
    ...
    this.http.get<any>(apiUrl, { withCredentials: true }).subscribe({
      next: (response) => {
        ...
        console.log('File types loaded for no_ad_copy check:', this.fileTypes);
      },
      error: (error) => {
