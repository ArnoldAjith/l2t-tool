...
@Component({
  selector: 'app-top-conversion',
  imports: [CommonModule, TitleCasePipe],
  templateUrl: './top-conversion.html',
  styleUrl: './top-conversion.css'
})
export class TopConversion implements OnInit {
  ...
  loadAdsData(): void {
    ...
    const sheetName = "Topconvertingkeywords";
    
    this.api.getScriptData(fileName, sheetName).subscribe({
      next: (response) => {
        console.log(response);
        this.adsData = response.Topconvertingkeywords || [];
        this.extractColumns();
        this.loading = false;
  ...
  exportToExcel(): void {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Top Conversion');
      
      const fileName = `top-conversion-${this.getCurrentDate()}.xlsx`;
