...
@Component({
  selector: 'app-check-for-campaigns-running-on',
  imports: [CommonModule, TitleCasePipe],
  templateUrl: './check-for-campaigns-running-on.html',
  styleUrl: './check-for-campaigns-running-on.css'
})
export class CheckForCampaignsRunningOn implements OnInit {
  ...
  loadAdsData(): void {
    ...
    const sheetName = "Check for Campaigns Running on ";
    
    this.api.getScriptData(fileName, sheetName).subscribe({
      next: (response) => {
        console.log(response);
        this.adsData = response.Check_for_Campaigns_Running_on_ || [];
        this.extractColumns();
        this.loading = false;
  ...
  exportToExcel(): void {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Check for Campaigns Running on');
      
      const fileName = `check-for-ampaigns-running-on-${this.getCurrentDate()}.xlsx`;
