...
@Component({
  selector: 'app-offensive-language-detection',
  standalone: true,
  imports: [CommonModule, FormsModule, HttpClientModule],
  templateUrl: './offensive-language-detection.html',
  styleUrls: ['./offensive-language-detection.css']
})
export class OffensiveLanguageDetection implements OnInit {
  ...
  loadFileTypes(): void {
    // Load file types specific to the 'offensive_language' check
    const checkKey = 'offensive_language';
    ...
    this.http.get<any>(apiUrl, { withCredentials: true }).subscribe({
      next: (response) => {
        ...
        console.log('File types loaded for offensive_language check:', this.fileTypes);
      },
      error: (error) => {
  ...
  runAudit() {
    ...
    if (this.dataSource === 'upload') {
      ...
      const check = 'offensive_language';
      const requestBody = {
