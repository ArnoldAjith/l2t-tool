...
@Component({
  selector: 'app-keyword-ad-copy-mismatch',
  standalone: true,
  imports: [CommonModule, FormsModule, HttpClientModule],
  templateUrl: './keyword-ad-copy-mismatch.html',
  styleUrls: ['./keyword-ad-copy-mismatch.css']
})
export class KeywordAdCopyMismatch implements OnInit {
  ...
  loadFileTypes(): void {
    // Load file types specific to the 'term_copy_mismatch' check
    const checkKey = 'term_copy_mismatch';
    ...
    this.http.get<any>(apiUrl, { withCredentials: true }).subscribe({
      next: (response) => {
        ...
        console.log('File types loaded for term_copy_mismatch check:', this.fileTypes);
      },
      error: (error) => {
  ...
  runAudit() {
    ...
    if (this.dataSource === 'upload') {
      ...
      const check = 'term_copy_mismatch';
      const requestBody = {
