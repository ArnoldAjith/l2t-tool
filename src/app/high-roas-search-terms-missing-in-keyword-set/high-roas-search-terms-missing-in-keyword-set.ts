...
@Component({
  selector: 'app-high-roas-search-terms-missing-in-keyword-set',
  standalone: true,
  imports: [CommonModule, FormsModule, HttpClientModule],
  templateUrl: './high-roas-search-terms-missing-in-keyword-set.html',
  styleUrls: ['./high-roas-search-terms-missing-in-keyword-set.css']
})
export class HighRoasSearchTermsMissingInKeywordSet implements OnInit {
  ...
  loadFileTypes(): void {
    // Load file types specific to the 'missing_terms' check
    const checkKey = 'missing_terms';
    ...
    this.http.get<any>(apiUrl, { withCredentials: true }).subscribe({
      next: (response) => {
        ...
        console.log('File types loaded for missing_terms check:', this.fileTypes);
      },
      error: (error) => {
