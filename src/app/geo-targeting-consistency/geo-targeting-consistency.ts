...
@Component({
  selector: 'app-geo-targeting-consistency',
  standalone: true,
  imports: [CommonModule, FormsModule, HttpClientModule],
  templateUrl: './geo-targeting-consistency.html',
  styleUrls: ['./geo-targeting-consistency.css']
})
export class GeoTargetingConsistency implements OnInit {
  ...
  loadFileTypes(): void {
    // Load file types specific to the 'geo_targeting' check
    const checkKey = 'geo_targeting';
    ...
    this.http.get<any>(apiUrl, { withCredentials: true }).subscribe({
      next: (response) => {
        ...
        console.log('File types loaded for geo_targeting check:', this.fileTypes);
      },
      error: (error) => {
