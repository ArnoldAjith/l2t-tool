...
@Component({
  selector: 'app-budget-allocation-efficiency',
  standalone: true,
  imports: [CommonModule, FormsModule, HttpClientModule],
  templateUrl: './budget-allocation-efficiency.html',
  styleUrls: ['./budget-allocation-efficiency.css']
})
export class BudgetAllocationEfficiency implements OnInit {
  ...
  loadFileTypes(): void {
    // Load file types specific to the 'budget' check
    const checkKey = 'budget';
    ...
    this.http.get<any>(apiUrl, { withCredentials: true }).subscribe({
      next: (response) => {
        ...
        console.log('File types loaded for budget check:', this.fileTypes);
      },
      error: (error) => {
