...
@Component({
  selector: 'app-search-partners-performance',
  standalone: true,
  imports: [CommonModule, FormsModule, HttpClientModule],
  templateUrl: './search-partners-performance.html',
  styleUrls: ['./search-partners-performance.css']
})
export class SearchPartnersPerformance implements OnInit {
  ...
  loadFileTypes(): void {
    // Load file types specific to the 'search_partners' check
    const checkKey = 'search_partners';
    ...
    this.http.get<any>(apiUrl, { withCredentials: true }).subscribe({
      next: (response) => {
        ...
        console.log('File types loaded for search_partners check:', this.fileTypes);
      },
      error: (error) => {
