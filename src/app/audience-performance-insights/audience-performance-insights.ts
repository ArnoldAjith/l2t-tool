...
@Component({
  selector: 'app-audience-performance-insights',
  standalone: true,
  imports: [CommonModule, FormsModule, HttpClientModule],
  templateUrl: './audience-performance-insights.html',
  styleUrls: ['./audience-performance-insights.css']
})
export class AudiencePerformanceInsights implements OnInit {
  ...
  loadFileTypes(): void {
    // Load file types specific to the 'audience' check
    const checkKey = 'audience';
    ...
    this.http.get<any>(apiUrl, { withCredentials: true }).subscribe({
      next: (response) => {
        ...
        console.log('File types loaded for audience check:', this.fileTypes);
      },
      error: (error) => {
