...
@Component({
  selector: 'app-device-segmentation-opportunities',
  standalone: true,
  imports: [CommonModule, FormsModule, HttpClientModule],
  templateUrl: './device-segmentation-opportunities.html',
  styleUrls: ['./device-segmentation-opportunities.css']
})
export class DeviceSegmentationOpportunities implements OnInit {
  ...
  loadFileTypes(): void {
    // Load file types specific to the 'device_cpl' check
    const checkKey = 'device_cpl';
    ...
    this.http.get<any>(apiUrl, { withCredentials: true }).subscribe({
      next: (response) => {
        ...
        console.log('File types loaded for device_cpl check:', this.fileTypes);
      },
      error: (error) => {
