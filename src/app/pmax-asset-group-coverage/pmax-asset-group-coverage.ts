...
@Component({
  selector: 'app-pmax-asset-group-coverage',
  standalone: true,
  imports: [CommonModule, FormsModule, HttpClientModule],
  templateUrl: './pmax-asset-group-coverage.html',
  styleUrls: ['./pmax-asset-group-coverage.css']
})
export class PmaxAssetGroupCoverage implements OnInit {
  ...
  loadFileTypes(): void {
    // Load file types specific to the 'ppmax_group' check
    const checkKey = 'ppmax_group';
    ...
    this.http.get<any>(apiUrl, { withCredentials: true }).subscribe({
      next: (response) => {
        ...
        console.log('File types loaded for ppmax_group check:', this.fileTypes);
      },
      error: (error) => {
